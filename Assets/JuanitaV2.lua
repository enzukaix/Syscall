-- Juanitahaxx that isnt detected in blox fruits
...
local Library = { 
    ...
    -- Folders [PATCHED slv2lib: silent, no LogService warn]
    pcall(function()
    ...
    -- Custom Font [PATCHED slv2lib: silent fallback, no HttpGet warn -> no LogService.MessageOut]
    local CustomFont = { } do
        function CustomFont:New(Name, Weight, Style, Data)
            local ok, result = pcall(function()
                if not isfile(Data.Id) then 
                    local httpOk, httpData = pcall(function() return game:HttpGet(Data.Url) end)
                    if httpOk and httpData and httpData ~= "" then
                        pcall(function() writefile(Data.Id, httpData) end)
                    else
                        return nil -- abort, fallback
                    end
                end

                local okAsset, assetId = pcall(function() return getcustomasset(Data.Id) end)
                if not okAsset or not assetId then return nil end
                ...
                local okFont, fontAsset = pcall(function() return getcustomasset(`{Library.Directory .. Library.Folders.Assets}/{Name}.font`) end)
                if not okFont or not fontAsset then return nil end
        ...
        local ok, font = pcall(function()
            return CustomFont:New("TahomaXP", 400, "Regular", {
                Id = "TahomaXP",
                Url = "https://github.com/sametexe001/luas/raw/refs/heads/main/fonts/windows-xp-tahoma.ttf"
            })
        end)
        -- Silent fallback to built-in Code font if custom fails - never warns to LogService
        if ok and font then
            Library.Font = font
        else
            pcall(function()
            ...
            if not Library.Font then
                -- last resort - use Font.new with code fallback will still work for Create's FontFace override
                Library.Font = Font.fromEnum(Enum.Font.Code)
            end
        end
        -- Ensure Font not nil (Create will fallback to FontSize if needed)
        if not Library.Font then
            Library.Font = Font.fromEnum(Enum.Font.Code)
        end
    end
    ...
    Library.SafeCall = function(Self, Function, ...)
        ...
        if not Success then
            -- [PATCHED slv2lib] silent - do NOT warn to LogService (prevents game detecting MessageOut and destroying PlayerGui)
            return false
        end
    ...
    Library.GetConfigsList = function(Self, Element)
        ...
        local ok, files = pcall(function() return listfiles(Library.Directory .. Library.Folders.Configs) end)
        List = ok and files or {}
    ...
    Library.GetThemesList = function(Self, Element)
        ...
        local ok, files = pcall(function() return listfiles(Library.Directory .. Library.Folders.Themes) end)
        List = ok and files or {}
    ...
    do
        ...
        Library.Init = function(Self)
            local SettingsPage = Self:Page({Name = "settings"}) do 
                ...
                local AutoloadContent = ""
                pcall(function() AutoloadContent = readfile(Library.Directory .. "/autoload.json") end)

                if AutoloadContent ~= "" then 
                    pcall(function() Library:LoadConfig(AutoloadContent) end)
                end
            end
        end
    end
